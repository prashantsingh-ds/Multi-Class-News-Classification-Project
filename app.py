...
#Downloading some dependencies (cached so this only runs once per app instance)
@st.cache_resource
def load_nltk_data():
...
def load_spacy_model():
    try:
        ...
        # Fallback in case the model wheel wasn't installed via requirements.txt
...
#Loading the DL Model (cached so it is not reloaded on every rerun)
@st.cache_resource
def load_model():
    return tf.keras.models.load_model('News_Classification_bidirectional_lstm_model.keras')

model = load_model()

# NewsAPI key: read from Streamlit secrets first, fall back to hardcoded value.
# IMPORTANT: put your key in .streamlit/secrets.toml (locally) or in the
# "Secrets" section of your Streamlit Cloud app settings, as NEWSAPI_KEY.
NEWSAPI_KEY = st.secrets.get('NEWSAPI_KEY', 'a1e91cf9073f4b85aa784fe5f37e6294')
...
def fetch_news(name, date_from, date_to):
    ...
    url = 'https://newsapi.org/v2/everything?q={}&from={}&to={}&language=en&sortBy=popularity&apiKey={}'.format(query, date_from, date_to, NEWSAPI_KEY)

    response = requests.get(url, headers=headers)
    data = response.json()

    if data.get('status') != 'ok':
        st.error("NewsAPI request failed: {}".format(data.get('message', 'Unknown error. Check your API key/quota.')))
        final = pd.DataFrame(columns=['name', 'title', 'description', 'content', 'url', 'img_url', 'Date'])
        final.to_csv('news.csv', index=False)
        return

    tot_res = data.get('totalResults', 0)
    ls_name = []
    ls_title = []
    ls_desc = []
    ls_url = []
    ...
    if tot_res == 0:
        st.warning("No articles found for this keyword/date range. Try a different keyword or date range.")
        final = pd.DataFrame(columns=['name', 'title', 'description', 'content', 'url', 'img_url', 'Date'])
        final.to_csv('news.csv', index=False)
        return

    # Number of pages available (NewsAPI returns up to 100 articles per page)
    num_pages = max(1, -(-tot_res // 100))  # ceiling division

    for i in tqdm(range(1, num_pages + 1)):
        try:
            for j in data.get('articles', []):
                article_name = j['source']['name']
                title = j['title']
                desc = j['description']
                article_url = j['url']
            ...
            if i < num_pages:
                next_page_url = 'https://newsapi.org/v2/everything?q={}&from={}&to={}&language=en&page={}&sortBy=popularity&apiKey={}'.format(query, date_from, date_to, i + 1, NEWSAPI_KEY)
...
def get_tokenizer():
    ...
    tok = Tokenizer()
    tok.fit_on_texts(train_df['tags'].astype(str))
    return tok
...
if st.button('Fetch!'):
    ...
    if not name:
        ...
        with st.spinner('Fetching and classifying news articles...'):
